...
void enableRawMode(){
  ...
  raw.c_oflag &= ~(OPOST);
  raw.c_cflag |= (CS8);
  raw.c_lflag &= ~(ECHO | ICANON | ISIG | ISIG); //ECHO COMO FROM <termios.h>.

  raw.c_cc[VMIN] = 0;
  raw.c_cc[VTIME] = 1;


  if (tcsetattr(STDIN_FILENO, TCSAFLUSH, &raw) == -1) die("tcsetattr");// tcsetattr come from <termios.h>.
}
...
/*
https://viewsourcecode.org/snaptoken/kilo/03.rawInputAndOutput.html#tildes
*/